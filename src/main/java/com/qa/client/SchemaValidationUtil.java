...
public class SchemaValidationUtil {
    ...
    /**
     * Method to perform schema validation with Response Object
     *
     * @param responseObject
     */
    public static String performSchemaValidation(Object responseObject, File inputSchemaFilePath) {
        Gson gson = new Gson();
        String json = gson.toJson(responseObject);

        try {
            if (isJsonValid(inputSchemaFilePath, json)) {
                Assert.assertTrue(true, "Schema validation successfull");
            } else {
                Assert.assertTrue(false, "Schema validation is not successfull");
            }
            ...
            Assert.assertFalse(true,
                    "Error processing schema. Please check the response if it has changed");
        } catch (IOException e) {
            e.printStackTrace();
            Assert.assertFalse(true, "Error reading file. File not found. File path sent in parameter: " + inputSchemaFilePath);
        }
    ...
    /**
     * Method to perform schema validation with Json String
     *
     * @param responseJson
     */
    public static String performSchemaValidation(String responseJson, File inputSchemaFilePath) {
        try {
            if (isJsonValid(inputSchemaFilePath, responseJson)) {
                Assert.assertTrue(true, "Schema validation successfull");
            } else {
                Assert.assertTrue(false, "Schema validation is not successfull");
            }
